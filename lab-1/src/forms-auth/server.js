...
fastify.addHook("onRequest", (request, reply, done) => {
  let sessionId = request.cookies[SESSION_KEY];

  if (!sessionId) {
    sessionId = fastify.sessionStorage.init();
    reply.cookie(SESSION_KEY, sessionId);
  }
...
fastify.post("/api/login", (request, reply) => {
  ...
  fastify.log.warn({ login, password });
  ...
  return reply.send({ username: user.username, token: request.ctx.sessionId });
});
